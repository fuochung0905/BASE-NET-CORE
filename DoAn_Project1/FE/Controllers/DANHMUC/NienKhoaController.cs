...
{
    public class NienKhoaController : BaseController<NienKhoaController>
    {
        public IActionResult Index()
        {
            return View("~/Views/DanhMuc/NienKhoa/Index.cshtml", GetPhanQuyen());
        }
        ...
        public IActionResult GetList([DataSourceRequest] DataSourceRequest request, GetListPagingRequest param)
        {
            try
            {
                var result = new List<MODELNienKhoa>();
                ...
                ResponseData response = this.PostAPI(URL_API.NIENKHOA_GETLIST, param);
                ...
                if (response.Status)
                {
                    ...
                    result = JsonConvert.DeserializeObject<List<MODELNienKhoa>>(dataResult.Data.ToString());
        ...
        public IActionResult GetListAll()
        {
            ...
            {
                var result = new List<MODELNienKhoa>();
                ResponseData response = this.PostAPI(URL_API.NIENKHOA_GETLIST, new GetAllRequest());

                if (response.Status)
                {
                    result = JsonConvert.DeserializeObject<List<MODELNienKhoa>>(response.Data.ToString());
                }
                else
                {
                    throw new Exception(response.Message);
            ...
            {
                return Json(new List<MODELNienKhoa>());
            }
        }
        ...
        public ActionResult ShowViewPopup(Guid id)
        {
            try
            {
                MODELNienKhoa obj = new MODELNienKhoa();

                if (id != null)
                {
                    ResponseData response = this.PostAPI(URL_API.NIENKHOA_GETBYID, new { Id = id });

                    if (response.Status)
                    {
                        obj = JsonConvert.DeserializeObject<MODELNienKhoa>(response.Data.ToString());
                    }
                }

                return PartialView("~/Views/DanhMuc/NienKhoa/PopupView.cshtml", obj);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult ShowInsertPopup()
        {
            try
            {
                PostNienKhoaRequest obj = new PostNienKhoaRequest();

                ResponseData response = this.PostAPI(URL_API.NIENKHOA_GETBYPOST, new { Id = Guid.Empty });

                if (response.Status)
                {
                    obj = JsonConvert.DeserializeObject<PostNienKhoaRequest>(response.Data.ToString());
                }

                return PartialView("~/Views/DanhMuc/NienKhoa/PopupDetail.cshtml", obj);
            }
            catch (Exception ex)
            {
                ViewBag.ErrorMessage = ex.Message;
        ...
        public ActionResult ShowUpdatePopup(Guid id)
        {
            try
            {
                ...
                ResponseData response = this.PostAPI(URL_API.NIENKHOA_GETBYPOST, new { Id = id });
        ...
        [HttpPost]
        public JsonResult Post([DataSourceRequest] DataSourceRequest dataSourceRequest, PostNienKhoaRequest param)
        {
            try
            {
                if (param != null && ModelState.IsValid)
                {
                    ...
                    if (param.IsEdit)
                    {
                        response = this.PostAPI(URL_API.NIENKHOA_UPDATE, param);
                    }
                    else
                    {
                        response = this.PostAPI(URL_API.NIENKHOA_INSERT, param);
                    }
                    if (!response.Status)
                    {
        ...
        public JsonResult Delete([DataSourceRequest] DataSourceRequest dataSourceRequest, List<Guid> listSelectedId)
        {
            try
            {
                ResponseData response = this.PostAPI(URL_API.NIENKHOA_DELETELIST, new { ids = listSelectedId }); ;
        ...
        {
            ResponseData response = this.PostAPI(URL_API.NIENKHOA_GETALLFORCOMBOBOX, new GetAllRequest());
