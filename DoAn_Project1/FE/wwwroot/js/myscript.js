    ...
    confirmDialog: function (message, title, buttons) {
        ...
        for (index = 0; index < buttons.length; ++index) {
            if (buttons[index].text == "Có") {
                contentHTML += "<button class='confirm btn btn-success btn-sm btn-class-confirm-" + index + "' tabindex='1' style='display: inline-block;'>" + "<i class='fas fa-check-square'></i> " + buttons[index].text + "</button>";
            }
            else {
                contentHTML += "<button class='cancel btn btn-default btn-sm btn-class-confirm-" + index + "' tabindex='" + (index + 1) + "' style='display: inline-block;'>" + "<i class='fas fa-times-square'></i> " + buttons[index].text + "</button>";
            }
        }
        contentHTML += "</div>";
        contentHTML += "</div>";
