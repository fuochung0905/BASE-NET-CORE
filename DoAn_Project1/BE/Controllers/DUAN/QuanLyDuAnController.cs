...
using REPONSITORY.DUAN.QUANLYDUAN;

namespace BE.Controllers.DUAN;

[Route("api/[controller]")]
[ApiController]
public class QuanLyDuAnController : ControllerBase
{
	private readonly IQUANLYDUANService _service;

	public QuanLyDuAnController(IQUANLYDUANService service)
	{
		_service = service;
	}

	[HttpPost, Route("get-list")]
    public IActionResult GetList(GetListQuanLyDuAnRequest request)
	{
		try
		{
    ...
    public IActionResult Insert(PostQuanLyDuAnRequest request)
	{
		try
		{
			if(!ModelState.IsValid)
    ...
    public IActionResult Update(PostQuanLyDuAnRequest request)
	{
		try
		{
			if(!ModelState.IsValid)
    ...
    public IActionResult GetAllComboBox(GetAllRequest request)
	{
		try
		{
			if(!ModelState.IsValid)
